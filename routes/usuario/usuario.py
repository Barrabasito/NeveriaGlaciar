...
def ver_usuario(id_usuario):
    ...
    usuario_data = {
        ...
        'nombre_usuario': usuario.nombre_usuario,
        'password': usuario.password,
        'fecha_registro': usuario.fecha_registro.strftime('%Y-%m-%d'),  # Formatear la fecha como string
        'admin': usuario.admin,
        'sucursal_id': usuario.sucursal_id
    }
    ...
    try:
        ...
        if request.method == "GET":
            ...
            if isAdmin:
                ...
                isAdmin = 0
            print(isAdmin)
            return render_template("editarUsuario.html", usuario=usuario, isAdmin=isAdmin, sucursales=sucursales)
            ...
            existing_user = Usuario.query.filter(Usuario.nombre_usuario == nombre_usuario, Usuario.id_usuario != id_usuario).first()
            if existing_user:
                responseObject = {"status": "error", "message": "El nombre de usuario ya está en uso"}
                return jsonify(responseObject)

            usuario.nombre_usuario = nombre_usuario
            usuario.password = bcrypt.generate_password_hash(password, BaseConfig.BCRYPT_LOG_ROUNDS).decode()
            usuario.admin = admin
            usuario.sucursal_id = sucursal_id

            db.session.commit()
            responseObject = {"status": "success", "message": "Usuario editado correctamente", "id_usuario": usuario.id_usuario}
            return jsonify(responseObject)

    except IntegrityError as e:
        db.session.rollback()
        responseObject = {"status": "error", "message": "Error de integridad en la base de datos"}
        return jsonify(responseObject)
