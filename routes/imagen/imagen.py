...
from models import ImagenEmpleado
from app import db
from auth import tokenCheck
import base64

ImagenEmpleadoUser = Blueprint("imageUser", __name__, template_folder="templates")
...
@ImagenEmpleadoUser.route("/displayImage", methods=["GET"])
@tokenCheck
def search_page(usuario):
    searchImage = ImagenEmpleado.query.filter_by(user_id=usuario["user_id"]).first()
    if searchImage:
        image = searchImage.rendered_data
        return render_template("PerfilUsuario.html", imagen=image)


@ImagenEmpleadoUser.route("/uploadPerfil", methods=["POST"])
@tokenCheck
def upload(usuario):
    searchImage = ImagenEmpleado.query.filter_by(user_id=usuario["user_id"]).first()
    if searchImage:
